...
if ( !function_exists("pryc_wp_add_custom_content_to_bottom_of_post") ) {
	...
	function pryc_wp_add_custom_content_to_bottom_of_post($content) {
		...
		if ( ( is_single() && is_main_query() ) || ( ( ( isset( $options['pryc_wp_add_custom_content_to_bottom_of_post_section3_checkbox_field_2'] ) ) && ( !empty( $options['pryc_wp_add_custom_content_to_bottom_of_post_section3_checkbox_field_2'] ) ) ) && ( is_singular() && is_main_query() ) ) && ( ( empty( $options['pryc_wp_add_custom_content_to_bottom_of_post_section3_text_field_1'] ) ) || ( !is_page( $page_id_exclude_array ) ) ) ) {
...
function pryc_wp_add_custom_content_to_bottom_of_post_section2_textarea_field_1_render() { 
	...
	<?php
	echo '<br /><br />';
	echo __( 'You may use text, HTML, Shortcodes and JavaScript. Don\'t use the \\ ("\\") character. If you must, use double \\\ instead of \\.', 'pryc_wp_add_custom_content_to_bottom_of_post' );
}
